...
class CameraController(DirectObject):
    ...
    def setupCamera(self):
        self.camAnchor = self.base.render.attachNewNode("Cam Anchor")
        self.base.camera.reparentTo(self.camAnchor)
        self.base.camera.setPos(0, -self.initZoom, 0)
        self.base.camera.lookAt(self.camAnchor)
    ...
    def setupTasks(self):
        self.base.taskMgr.add(self.cameraOrbit, "Camera Orbit")
        self.base.taskMgr.add(self.cameraZoom, "Camera Zoom")
        self.base.taskMgr.add(self.cameraMove, "Camera Move")
    ...
    def cameraOrbit(self, task):
        if self.orbit != None:
            if self.base.mouseWatcherNode.hasMouse():
                ...
                self.base.win.movePointer(0, int(self.orbit[1][0]), int(self.orbit[1][1]))
    ...
    def cameraZoom(self, task):
        if (self.zoom != None):
            if self.base.mouseWatcherNode.hasMouse():
                ...
                self.base.win.movePointer(0, int(self.zoom[1][0]), int(self.zoom[1][1]))

                deltaY = (mpos[1] - self.zoom[0][1]) * self.base.camera.getY()
